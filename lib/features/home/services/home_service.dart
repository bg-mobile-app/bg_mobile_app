    ...
    try {
      ...
      return rawList.map((json) {
        return WorkPermitItem(
          ...
          slug: json['slug'] ?? '',
          image:
              json['image'] ?? 'assets/img/work-permit/1.jpg', // fallback image
          customerPrice: json['customerPrice'] is int
              ? json['customerPrice']
              : (int.tryParse(json['customerPrice']?.toString() ?? '0') ??
                    int.tryParse(json['customer_price']?.toString() ?? '0') ??
                    0),
          agentPrice: json['agentPrice'] is int
              ? json['agentPrice']
              : (int.tryParse(json['agentPrice']?.toString() ?? '0') ??
                    int.tryParse(json['agent_price']?.toString() ?? '0') ??
                    0),
          countryName: json['countryName'] ?? json['country_name'] ?? 'Unknown',
          countryFlag:
              json['countryFlag'] ??
              json['country_flag'] ??
              'assets/img/customer/appointment/world.png',
  ...
  Future<List<WorkPermitItem>> filterWorkPermits({
    ...
    try {
      ...
      return rawList.map((json) {
        ...
        final countryName = countryData is Map
            ? countryData['name']
            : (json['countryName'] ?? json['country_name']);
        final countryFlag = countryData is Map
            ? countryData['flag']
            : (json['countryFlag'] ?? json['country_flag']);
